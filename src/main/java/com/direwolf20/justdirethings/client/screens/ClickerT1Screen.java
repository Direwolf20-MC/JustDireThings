...
public class ClickerT1Screen extends BaseMachineScreen<ClickerT1Container> {
    ...
    public void addTickSpeedButton() {
        ...
        if (clickType == 2) {
            tickSpeedButton = ToggleButtonFactory.TICKSPEEDBUTTON(leftPos + 144, topSectionTop + 40, tickSpeed, maxHoldTicks + 1, b -> {
                tickSpeed = ((NumberButton) b).getValue();
                ClientPacketDistributor.sendToServer(new TickSpeedPayload(tickSpeed));
            });
        } else {
            tickSpeedButton = ToggleButtonFactory.TICKSPEEDBUTTON(leftPos + 144, topSectionTop + 40, tickSpeed, b -> {
                tickSpeed = ((NumberButton) b).getValue();
                ClientPacketDistributor.sendToServer(new TickSpeedPayload(tickSpeed));
            });
        }
    ...
    public void init() {
        ...
        addRenderableWidget(ToggleButtonFactory.CLICKTARGETBUTTON(leftPos + 56, topSectionTop + 38, clickTarget, b -> {
            clickTarget = ((ToggleButton) b).getTexturePosition();
            saveSettings();
        }));

        addRenderableWidget(ToggleButtonFactory.LEFTRIGHTCLICKBUTTON(leftPos + 38, topSectionTop + 38, clickType, b -> {
        ...
        maxHoldTicksButton = new NumberButton(leftPos + 34, topSectionTop + 55, 24, 12, maxHoldTicks, 1, 1200, Component.translatable("justdirethings.screen.click-hold-for"), b -> {
            maxHoldTicks = ((NumberButton) b).getValue();
            if (clickType == 2) {
                tickSpeed = Math.max(tickSpeed, maxHoldTicks + 1);
                ClientPacketDistributor.sendToServer(new TickSpeedPayload(tickSpeed));
            }
        ...
        addRenderableWidget(ToggleButtonFactory.SNEAKCLICKBUTTON(leftPos + 20, topSectionTop + 38, sneaking, b -> {
            sneaking = !sneaking;
            ((GrayscaleButton) b).toggleActive();
            saveSettings();
        }));

        addRenderableWidget(ToggleButtonFactory.SHOWFAKEPLAYERBUTTON(leftPos + 2, topSectionTop + 38, showFakePlayer, b -> {
    ...
    public void saveSettings() {
        super.saveSettings();
        ClientPacketDistributor.sendToServer(new ClickerPayload(clickType, clickTarget, sneaking, showFakePlayer, maxHoldTicks));
    }
}
