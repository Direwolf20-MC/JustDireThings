...
public class BlockPlacerT2Container extends BaseMachineContainer {
    ...
    public BlockPlacerT2Container(int windowId, Inventory playerInventory, BlockPos blockPos) {
        super(JDTRegistration.BlockPlacerT2_Container.get(), windowId, playerInventory, blockPos);
        addPlayerSlots(player.getInventory());
    }

    @Override
    public boolean stillValid(Player playerIn) {
        return stillValid(ContainerLevelAccess.create(player.level(), pos), player, JDTRegistration.BlockPlacerT2.get());
    }
