    ...
    public void handle(final AreaAffectingPayload payload, final PlayPayloadContext context) {
        context.workHandler().submitAsync(() -> {
            ...
            if (container instanceof AreaAffectingContainer areaAffectingContainer) {
                areaAffectingContainer.areaAffectingBE.setAreaSettings(payload.xRadius(), payload.yRadius(), payload.zRadius(), payload.xOffset(), payload.yOffset(), payload.zOffset(), payload.renderArea(), payload.redstoneMode());
            }
        });
    }
}
