...
public class AbilityRecipe implements CraftingRecipe {
    protected final ItemStack input;
    protected final ItemStack output;
    protected String abilityRequirement;
    
    public AbilityRecipe(ItemStack input, ItemStack output, String abilityRequirement) {
        this.input = input;
        this.output = output;
    ...
    public RecipeType<?> getType() {
        return Registration.ABILITY_RECIPE_TYPE.get();
    }
    ...
    public static class Serializer implements RecipeSerializer<AbilityRecipe> {
        ...
        private static final Codec<AbilityRecipe> CODEC = RecordCodecBuilder.create(
        ...
        @Override
        public Codec<AbilityRecipe> codec() {
            return CODEC;
        }

        public AbilityRecipe fromNetwork(FriendlyByteBuf pBuffer) {
        	ItemStack inputState = pBuffer.readItem();
        	ItemStack outputState = pBuffer.readItem();
        	String abilityRequirement = pBuffer.readUtf(256);

            return new AbilityRecipe(inputState, outputState, abilityRequirement);
        }

        public void toNetwork(FriendlyByteBuf pBuffer, AbilityRecipe pRecipe) {
